...
public class QueryFour {
    ...
    public static void runQ4() {
        ...
        System.out.println("Enter the Customer ID for Query 4");
        ...
        customerResult.forEach((Block<? super Document>) d -> {
            ...
            orderCollection.find(Filters.eq("customer", d.get("id"))).forEach((Block<? super Document>) m -> {
                ...
                paymentCollection.find(Filters.eq("id", m.get("payment"))).forEach((Block<? super Document>) t -> {
                    System.out.println("Payment ID: " + t.get("id"));
                    System.out.println("Payment Date: " + t.get("paymentDate"));
                });
            });
        });
    }
